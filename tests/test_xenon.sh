...
for expected in 'double_free: 1' 'invalid_free: 1' 'use_after_free: 1' 'out_of_bounds: 1' 'invalid_access: 1' 'overwritten_alloc: 1' 'leaks: 2'; do
...
set +e
"$BIN" analyze examples/with_issues.trace --json > /tmp/xenon_issues.json
json_code=$?
set -e

if [[ "$json_code" -ne 2 ]]; then
  echo "expected exit code 2 for json issue trace, got $json_code" >&2
  cat /tmp/xenon_issues.json >&2
  exit 1
fi

for expected in '"double_free": 1' '"invalid_free": 1' '"use_after_free": 1' '"out_of_bounds": 1' '"invalid_access": 1' '"overwritten_alloc": 1' '"leaks": 2'; do
  if ! grep -q "$expected" /tmp/xenon_issues.json; then
    echo "missing expected json summary line: $expected" >&2
    cat /tmp/xenon_issues.json >&2
    exit 1
  fi
done
