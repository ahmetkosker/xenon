...
JSON_OUT="$(mktemp)"
JSON_ERR="$(mktemp)"
trap 'rm -f "$CLEAN_OUT" "$ISSUES_OUT" "$OVERFLOW_OUT" "$LONG_PTR_OUT" "$MALFORMED_OUT" "$JSON_OUT" "$JSON_ERR"' EXIT
...
for expected in 'malformed_line: 1' 'invalid_number: 3' 'unknown_operation: 1'; do
...
set +e
"$BIN" analyze --format json examples/malformed.trace > "$JSON_OUT" 2> "$JSON_ERR"
code=$?
set -e

if [[ "$code" -ne 2 ]]; then
  echo "expected exit code 2 for malformed json trace, got $code" >&2
  cat "$JSON_OUT" >&2
  exit 1
fi

if [[ -s "$JSON_ERR" ]]; then
  echo "expected no stderr in json mode" >&2
  cat "$JSON_ERR" >&2
  exit 1
fi
...
for expected in '"malformed_line": 1' '"invalid_number": 3' '"unknown_operation": 1'; do
  if ! grep -q "$expected" "$JSON_OUT"; then
    echo "missing expected malformed json field: $expected" >&2
    cat "$JSON_OUT" >&2
    exit 1
  fi
done
