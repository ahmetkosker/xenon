...
- overwritten allocations (same pointer allocated again before `free`)
...
./build/xenon analyze examples/with_issues.trace --json
```
...
- `--json` disables per-event logs and emits a machine-readable summary suitable for CI parsers.
- Out-of-bounds detection is overflow-safe (`offset + len` will not wrap silently).
